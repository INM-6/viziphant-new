...
def plot_unitary_event_full_analysis(data, joint_suprise_dict, joint_suprise_significance, binsize, window_size, window_step, n_neurons,
    ...
    data: list of spiketrains
        list of spiketrains in different trials as representation of neural activity
    ...
    """
    try:
        _checkungUserEntries_plot_UE(data, joint_suprise_dict, joint_suprise_significance, binsize, 
        window_size, window_step, n_neurons, plot_params_user, plot_markers_user, position)
    except (TypeError, KeyError) as errors:
        print(errors)
        raise errors
    """
                                                                                                                        #pat = ue.inverse_hash_from_pattern(pattern_hash, n_neurons) # base fehlt?! ~/anaconda3/envs/vizitest/lib/python3.7/site-packages/elephant/unitary_event_analysis.py in inverse_hash_from_pattern(h, n_neurons, base)
    # figure format
    ...
    if len(plot_params['unit_real_ids']) != n_neurons:
        raise ValueError('length of unit_ids should be equal to number of neurons! \n'
                         'Unit_Ids: ' + plot_params['unit_real_ids'] +'ungleich NumOfNeurons: ' + n_neurons)
    ...
    plot_spike_events(data, window_size, window_step, n_neurons, plot_params_user, plot_markers_user[0],
                      position[0])

    plot_spike_rates(data, joint_suprise_dict, window_size, window_step, n_neurons,
                     plot_params_user, plot_markers_user[1], position[1])

    plot_coincidence_events(data, joint_suprise_dict, binsize, window_size, window_step, n_neurons,
                            plot_params_user, plot_markers_user[2], position[2])

    plot_coincidence_rates(data, joint_suprise_dict, window_size, window_step, n_neurons,
                           plot_params_user, plot_markers_user[3], position[3])

    plot_statistical_significance(data, joint_suprise_dict, joint_suprise_significance, window_size, window_step,
                                  n_neurons, plot_params_user, plot_markers_user[4], position[4])
...
def plot_spike_events(data, window_size, window_step, n_neurons, plot_params_user, plot_markers_user, position):
    ...
    Spike events occure when neurons get triggered from their neighbour neurons,
    so that they start firering themselves. This firing can be measured with electrods.
...
TODO: just one tupel neede, same for the other singel function calls
    ...
    anzahl_y_ticks_pro_Neuron = math.floor(num_tr/15)
    for i in range(anzahl_y_ticks_pro_Neuron):
...
def plot_coincidence_events(data, joint_suprise_dict, binsize, window_size, window_step, n_neurons,
    ...
    Coincidence events occur, when the observed neurons fire almost simultaneously,
    with a high probability of correlation.
    ...
    for n in range(n_neurons):
        for tr, data_tr in enumerate(data):
            ...
            ax2.plot(numpy.unique(joint_suprise_dict['indices']['trial' + str(tr)]) * binsize,
                     numpy.ones_like(numpy.unique(joint_suprise_dict['indices']['trial' + str(tr)]))
                     * tr + n * (num_tr + 1) + 1,
...
def plot_coincidence_rates(data, joint_suprise_dict, window_size, window_step, n_neurons, plot_params_user,
    ...
    There are two different rates:
        -emirical: represent the experimental measure results of the coincidence rates
        -expected: represent the theoretical calculus results of the coincidence rates
    ...
    ax3.plot(t_winpos + window_size / 2.,
             joint_suprise_dict['n_emp'] / (window_size.rescale('s').magnitude * num_tr),
             label='empirical', lw=plot_params['lw'], color=plot_markers['data_markercolor'][0])
    ax3.plot(t_winpos + window_size / 2.,
             joint_suprise_dict['n_exp'] / (window_size.rescale('s').magnitude * num_tr),
...
def plot_statistical_significance(data, joint_suprise_dict, joint_suprise_significance, window_size, window_step, n_neurons,
                                  plot_params_user, plot_markers_user, position):
    """
    Visualization of the statistical significance of the Unitary Event Analysis.
    ...
    ax4.plot(t_winpos + window_size / 2., joint_suprise_dict['Js'], lw=plot_params['lw'],
    ...
    ax4.axhline(joint_suprise_significance, ls='-', color=plot_markers['data_markercolor'][1])
...
def plot_unitary_events(data, joint_suprise_dict, joint_suprise_significance, binsize, window_size, window_step,
                        n_neurons, plot_params_user, plot_markers_user, position):
    """
    ...
    for n in range(n_neurons):
        for tr, data_tr in enumerate(data):
            ...
            sig_idx_win = numpy.where(joint_suprise_dict['Js'] >= joint_suprise_significance)[0]
            if len(sig_idx_win) > 0:
                x = numpy.unique(joint_suprise_dict['indices']['trial' + str(tr)])
                if len(x) > 0:
                    xx = []
...
def _checking_user_entries_of_plot_UE(data, joint_suprise_dict, joint_suprise_significance, binsize, window_size, window_step,
    ...
    if (type(joint_suprise_dict) != dict):
    ...
    if ( (type(joint_suprise_significance) != list) and (type(joint_suprise_significance) != numpy.float64)
            and (type(joint_suprise_significance) != numpy.ndarray)):
        raise TypeError('joint_suprise_significance must be a list (of floats)')
    elif (type(joint_suprise_significance) == list):
        for i in joint_suprise_significance:
            if ( (type(joint_suprise_significance[i]) != numpy.float64) and (type(joint_suprise_significance[i]) != float)):
                raise TypeError('elements of the joint_suprise_significance list are NOT floats')

    if (type(binsize) != pq.quantity.Quantity): #quantity scaler
        raise TypeError('binsize must be a quantity scaler/int')

    if (type(window_size) != pq.quantity.Quantity):
        raise TypeError('window_size must be a quantity scaler/int')

    if (type(window_step)!= pq.quantity.Quantity):
    ...
    if  ( (type(plot_markers_user) != list) and (type(plot_markers_user) != numpy.ndarray) ):
        raise TypeError("plot_markers_user must be a list")
    else:
        for i in plot_markers_user:
            if (type(plot_markers_user[i] != dict)):
                raise TypeError('elements of th plot_markers_user list are NOT dictionaries')
